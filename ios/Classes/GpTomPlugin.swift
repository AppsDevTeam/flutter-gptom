...
public final class GpTomPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
  ...
  private var iosRedirectUrl: String?
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    let events = FlutterEventChannel(name: Channels.events, binaryMessenger: registrar.messenger())
  ...
  private func handleInit(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
    ...
    iosRedirectUrl = args[JsonKeys.iosRedirectUrl] as? String
  ...
  private func validatePluginState() -> PluginResponse? {
    if !isInitialized {
      return .error(code: ResultCodes.notInitialized, message: "Call GpTomManager.init() first")
    }

    if iosRedirectUrl == nil || iosRedirectUrl!.isEmpty {
      return .error(
        code: ResultCodes.invalidArgument, message: "\(JsonKeys.iosRedirectUrl) is required on iOS")
    }
  ...
  private func register(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
    ...
    GpTomLog.i("register()", ["originRef": originRef, "clientId": clientId, "persist": persist])
  ...
  private func transaction(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
    ...
    guard let txId = args[JsonKeys.transactionId] as? String, !txId.isEmpty else {
      sendMethodResult(
        result,
        .error(code: ResultCodes.invalidArgument, message: "\(JsonKeys.transactionId) is required"))
      return
    }
    guard let typeNum = args[JsonKeys.transactionType] as? NSNumber else {
      sendMethodResult(
        result,
        .error(
          code: ResultCodes.invalidArgument, message: "\(JsonKeys.transactionType) is required"))
      return
    }

    let typeCode = typeNum.intValue
    guard let kind = Kinds.fromTransactionType(typeCode) else {
      let v = typeCode.map(String.init) ?? "null"
      sendMethodResult(
        result,
        .error(
          code: ResultCodes.invalidArgument,
          message: "Unsupported \(JsonKeys.transactionType) value: \(v)"))
      return
    }
      ...
      if cancelMode == 2 {
        ...
        if originTx == nil || originTx!.isEmpty {
          sendMethodResult(
            ...
            .error(
              code: ResultCodes.invalidArgument,
              message: "\(JsonKeys.originTransactionId) is required for olderTransaction cancel"))
          return
        }
      }
    }
    ...
    var params: [String: String?] = [
      ...
      DeeplinkParamKeys.redirectUrl: iosRedirectUrl,
    ...
    let url = DeeplinkBuilder.build(path: path, params: params)
    openDeeplink(url) { error in
      sendEvent(
        kind: kind,
        transactionId: txId,
        response: error
    ...
    GpTomLog.i(
      "transaction() opened deeplink", ["kind": kind, "txId": txId, "url": url.absoluteString])
  }
  ...
  private func openDeeplink(
    _ url: URL,
    onError: @escaping (PluginResponse.Error) -> Void
  ) {
    runOnMain {
  ...
  private func boolStr(_ v: Bool) -> String { v ? "true" : "false" }
